...
@lru_cache(maxsize=8)
def _load_demo_data(class_ids: tuple[int, ...], model_features: tuple[str, ...]) -> pd.DataFrame:
    ...
    for chunk in pd.read_csv(DATA_PATH, usecols=use_columns, chunksize=25_000):
        subset = chunk[chunk["student_class_id"].isin(class_ids)].copy()
        if not subset.empty:
...
class AnalyticsService:
    ...
    def student_summary(self, class_id: int, student_id: int) -> DashboardSummary:
        ...
        frame = class_frame[class_frame["user_id"] == student_id]
        if frame.empty:
        ...
        change = (recent_rate - earlier_rate) * 100
        hint_rate = float((recent["hint_count"] > 0).mean())
        model_probability = float(self.model.predict_probability(frame.tail(1))[0])
        skills = self._skill_metrics(recent, earlier, include_students=False)
        focus = min(skills, key=lambda item: item.success_rate) if skills else None
        focus_text = focus.label if focus else "the recent practice set"
        ...
        return DashboardSummary(
            ...
            cards=[
                MetricCard(
                    label="XGBoost success estimate",
                    value=_percent(model_probability),
                    detail="For the latest modeled interaction",
                    tone="positive"
                    if model_probability >= self.model.threshold
                    else "attention"
                    if model_probability < self.model.threshold
                    else "neutral",
                ),
                ...
                MetricCard(
                    label="Used at least one hint",
                    value=_percent(hint_rate),
                    detail="Of recent interactions",
                ),
                MetricCard(
                    label="Change from earlier",
                    value=_points(change),
                    detail="Compared with the preceding interactions",
                    tone="positive" if change >= 3 else "attention" if change <= -3 else "neutral",
                ),
            ],
            skills=skills,
            trend=_trend_points(frame, bins=7),
            suggested_questions=[
                "What pattern should I notice?",
                "Where is this student making progress?",
                "Which skill might benefit from a quick check-in?",
                "Suggest questions I can ask this student.",
            ],
            evidence=self._student_evidence(
                frame,
                recent,
                earlier,
                skills,
    ...
    def _student_evidence(
        ...
        evidence = [
            (
                f"The summary uses {len(frame)} recorded interactions across "
                f"{frame['skill_name'].nunique()} skills."
            ),
            (
                f"The persisted XGBoost model estimates {_percent(model_probability)} "
                f"first-attempt success for the latest modeled interaction; the support "
                f"signal is {model_threshold:.3f}."
            ),
            (
                f"Recent observed first-attempt success is {_percent(recent_rate)}, "
                f"{_points((recent_rate - earlier_rate) * 100)} from the preceding window."
            ),
            f"At least one hint was used in {_percent(hint_rate)} of recent interactions.",
        ]
        if skills:
            lowest = min(skills, key=lambda item: item.success_rate)
            evidence.append(
